...
- Yes the variables are in the correct format. 
...
-   Inspect the range values for each variable. What is the date range? The range of cases and deaths?
  - Date range is 06/01/2021 -> 03/23/2023. 
  - Range of cases is is 24224 - 12170765
  - Range of deaths is 255 - 105245
...
-   Use `plotly` for EDA: See if there are outliers or values that don't make sense for `new_cases` and `new_deaths`. Which states and which dates have strange values?
 ...
 - New York 11/13/2022 -> 11/17/2022 had over 500 new deaths. I know New York was hit hard by covid, so this somewhat makes sense. Perhaps the delta strain.
 - Florida had 445 deaths on 9/20/2023, which was another outlier. Florida and New York have very high state populations, so their higher death counts make sense.
 - In terms of dates, 6/7/2021 had greatly higher numbers of new deaths for all states compared to the rest of the dates listed. These do seem like outliers. 
...
cv_states_today_filter %>% 
  plot_ly(x = ~pop_density, y = ~deathsper100k,
          type = 'scatter', mode = 'markers', color = ~'state',
          size = ~population, sizes = c(5, 50), marker = list(sizemode='diameter', opacity=0.5),
          hoverinfo = 'text',
...
-   Explore the pattern between $x$ and $y$ using `geom_smooth()`
    ...
    - From the correlation slope, there appears to be a slightly inverse relationship between population density and deaths per 100k. Perhaps higher population density may attract more healthcare providers and more specialized care, and more resources, which may transfer to better outcomes. Lower population density may result in poor access to care. 
...
p <- ggplot(cv_states_today_filter, aes(x=pop_density, y=newdeathsper100k, size=population)) + 
  ...
  labs(title = "Scatterplot: Pop Density vs. New Deaths per 100k",
...
-   Create one more line chart, for Florida only, which shows `new_cases` and `new_deaths` together in one plot. Hint: use `add_layer()`
    ...
    - Time delay appears to be 1 month between peaks of cases and peaks of deaths. 
...
cv_states %>%
  filter(state == "Florida") %>% filter(date > "2021-06-15") %>% 
...
- California, Texas, Florida, and New York stand out for new cases. 
- Rhode Island, New York, Alaska, and Kentucky stand out for new cases per 100k
...
- States in the midwest ahd the south had the highest CFR. 
...
# Create hover text
cv_per100$hover <- with(cv_per100, paste(state_name, '<br>', "Cases per 100k: ", newper100k, '<br>', "Cases: ", cases, '<br>', "Deaths: ", deaths, "Case Fatality Rate: ", naive_CFR, '<br>'))
...
fig <- fig %>% colorbar(title = paste0("Case Fatality Rate: ", pick.date), limits = c(0,shadeLimit))
fig <- fig %>% layout(
    title = paste('Case Fatality Rate ', pick.date, '<br>(Hover for value)'),
    geo = set_map_details
  )
fig_pick.date <- fig
...
fig <- fig %>% colorbar(title = paste0("Case Fatality Rate: ", Sys.Date()), limits = c(0,shadeLimit))
fig <- fig %>% layout(
    title = paste('Case Fatality Rate', Sys.Date(), '<br>(Hover for value)'),
    geo = set_map_details
  )
fig_Today <- fig
